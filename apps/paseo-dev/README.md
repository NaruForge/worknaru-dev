...
```powershell
$env:WORKNARU_DATA_DIR = Join-Path (Get-Location).Path '.local/우리 팀 데이터'
pnpm web:dev
...
다른 터미널에서도 같은 루트를 사용하려면 그 터미널에도 변수를 설정한다. 값은 완전한 절대경로여야 하며 빈 값·상대경로·앞뒤 공백·쓰기 불가 경로는 오류다. 잘못된 값을 기본 폴더로 대체하지 않는다. 새 디렉터리는 생성할 수 있지만 기존 데이터·설정·ID를 자동 탐색·이전·덮어쓰기하지 않는다. 다른 루트는 별도 데이터이므로 기존 인스턴스가 자동 승계되지 않는다.
