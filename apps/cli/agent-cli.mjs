...
export async function runAgents(args, env, output) {
  ...
  try {
    ...
    const select = async selector => {
      ...
      const list = await agents.list(); output.stderr(format(list));
      return ask('Agent 이름 또는 ID: ');
    };
    if (p[0] === 'settings') {
      if (p[1] === 'get' && p.length <= 3 && (!p[2] || p[2] === 'send-mode')) { emit(await agents.settings()); return 0; }
      if (p[1] === 'set' && p[2] === 'send-mode') { expect(4); const current = await agents.settings(); emit(await agents.saveSettings({ sendMode: p[3], revision: current.revision })); return 0; }
    ...
    if (command === 'create') {
      ...
      expect(3); result = await agents.history({ agent: p[2] });
      while (o['--all'] && result.cursor) { const earlier = await agents.history({ agent: p[2], cursor: result.cursor }); result = earlier.epoch === result.epoch ? { ...earlier, entries: [...earlier.entries, ...result.entries] } : earlier; }
    } else if (command === 'queue') {
      if (p[2] === 'list') { expect(4); result = await agents.requests({ agent: p[3] }); }
      else if (p[2] === 'cancel') { expect(5); result = await agents.cancel({ agent: p[3], id: p[4] }); }
      else if (p[2] === 'resume') { expect(4); result = await agents.resume({ agent: p[3] }); }
      else if (p[2] === 'discard') { expect(5); if (!o['--yes']) { emit({ confirmationRequired: true, message: '대화와 파일을 확인한 뒤 --yes를 붙이면 불명확한 요청을 취소 처리합니다. 완료로 간주하거나 재전송하지 않습니다.' }); return 1; } result = await agents.discard({ agent: p[3], id: p[4] }); }
      ...
      result = await agents.permission({ agent: p[3], id: p[4], behavior: o['--allow'] ? 'allow' : 'deny',
        ...(o['--action'] ? { actionId: o['--action'] } : {}), ...(o['--answers'] ? { answers: JSON.parse(o['--answers']) } : {}) });
    } else if (command === 'archive') {
      expect(3); const preview = await agents.archivePreview({ agent: p[2] });
      if (!o['--yes']) {
      ...
      result = await agents.archive({ token: preview.token }); emit(result); return result.failed.length ? 1 : 0;
      ...
      if (command === 'send') request = await agents.send({ agent: selected, id: o['--id'] ?? crypto.randomUUID(), text: p[3],
        target: o['--project'] ? { type: 'project', projectId: o['--project'] } : o['--workspace'] ? { type: 'workspace', workspaceId: o['--workspace'] } : { type: 'standalone' } });
      ...
      try {
        while (Date.now() < deadline && !abort.signal.aborted) {
          const [list, current] = await Promise.all([agents.requests({ agent: selected }), agents.show({ agent: selected })]);
          ...
          if (['completed', 'failed', 'canceled', 'uncertain'].includes(request.state)) {
            emit(await requestReply(agents, selected, request));
          ...
          if (current.permissions.length) {
            ...
            await agents.permission({ agent: selected, id: permission.id, behavior: answer.toLowerCase() === 'y' ? 'allow' : 'deny', ...(answers ? { answers } : {}) });
          }
...
export async function requestReply(agents, agent, request) {
  if (!request.turnId) return { request, entries: [] };
  let history = await agents.history({ agent });
  const seen = new Set();
  let start = history.entries.findIndex(e => e.messageId === request.id && e.type === 'user_message');
  while (start < 0 && history.cursor && !seen.has(JSON.stringify(history.cursor))) {
    seen.add(JSON.stringify(history.cursor));
    const earlier = await agents.history({ agent, cursor: history.cursor });
    if (earlier.epoch !== history.epoch) break;
    history = { ...earlier, entries: [...earlier.entries, ...history.entries] };
    start = history.entries.findIndex(e => e.messageId === request.id && e.type === 'user_message');
  }
  return { request, entries: start < 0 ? [] : history.entries.slice(start + 1).filter(e => e.turnId === request.turnId && e.type === 'assistant_message'),
    ...(start < 0 ? { note: '이 요청의 응답 범위를 확인하지 못했습니다. agent history <Agent> --all로 대화 기록을 확인해 주세요.' } : {}) };
}

export function format(value) {
  if (Array.isArray(value)) return value.length ? value.map(v => v.name ? `${v.name}  ${v.id}  ${v.archivedAt ? '보관됨' : v.status}\n  ${v.cwd}`
    : v.kind && v.input && v.actions ? `${v.title || '응답이 필요합니다'}\n권한 ID: ${v.id}\n${v.description}\n입력: ${JSON.stringify(v.input, null, 2)}\n${v.actions.map(a => `선택: ${a.label} (--${a.behavior} --action ${a.id})`).join('\n')}`
    : `${v.title ?? v.id ?? ''}\n${v.description ?? ''}`).join('\n') + '\n' : '항목이 없습니다.\n';
  if (value?.error) return `${value.error.message}\n`;
  if (value?.entries) return (value.request ? `요청: ${value.request.state}\n${value.request.error ? `${value.request.error}\n` : ''}` : '') + conversationMessages(value.entries).map(e => `[${e.type === 'user_message' ? '사용자' : e.type === 'assistant_message' ? 'Agent' : e.type}] ${e.text}`).join('\n') + `\n${value.note ? `${value.note}\n` : ''}`;
