...
async function story(page: Page, name: string) {
  ...
  await expect(
    page.getByRole("button", { name: "새 Agent", exact: true }),
  ).toBeVisible();
}
...
for (const width of [390, 768, 1440])
  for (const colorScheme of ["light", "dark"] as const) {
    test(`conversation, dialogs and accessibility ${width} ${colorScheme}`, async ({
      page,
    }) => {
      ...
      await select(page);
      await expect(
        page.getByText("이번 주에는 Agent 생성", { exact: false }),
      ).toBeVisible();
      expect(
        await page.evaluate(
      ...
      if (process.platform === "linux")
        await expect(page).toHaveScreenshot(
          `conversation-${width}-${colorScheme}.png`,
          { animations: "disabled" },
        );
      await page
        .getByRole("button", { name: "Agent 상세 정보", exact: true })
        .click();
      await expect(
        page.getByText("C:\\Projects\\my-work", { exact: true }),
      ).toBeVisible();
      if (width < 1280) {
      ...
      if (process.platform === "linux")
        await expect(page).toHaveScreenshot(
          `archive-${width}-${colorScheme}.png`,
          { animations: "disabled" },
        );
    });
  }
test("a person can create, converse, follow up and archive", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText("요청을 확인했습니다: 첫 번째 요청", { exact: true }),
  ).toBeVisible();
...
test("mobile navigation keeps the draft; polling keeps focus", async ({
  page,
}) => {
...
test("permission and uncertain recovery require explicit actions", async ({
  page,
}) => {
  await story(page, "permission");
  await select(page);
  await expect(
    page.getByRole("button", { name: "이번 요청 승인" }),
  ).toBeVisible();
  expect((await new AxeBuilder({ page }).analyze()).violations).toEqual([]);
  ...
  await page
    .getByRole("dialog")
    .getByRole("button", { name: "기록 확인 후 실행 포기" })
    .click();
  await page.getByRole("button", { name: "남은 대기열 재개" }).click();
  await expect(
    page.getByText("자동 실행이 멈췄습니다.", { exact: false }),
  ).toHaveCount(0);
});
test("reading position survives refresh and content does not execute HTML", async ({
  page,
}) => {
...
test("empty, loading and disconnected screens remain readable on mobile", async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 844 });
  for (const name of ["empty", "loading", "error"]) {
    ...
    if (name === "error")
      await expect(
        page.getByRole("button", { name: "다시 연결" }),
      ).toBeVisible();
  }
});
