...
test("UI import and inline style violations fail with an actionable explanation", async () => {
  ...
  assert.ok(
    result.messages.some(
      (message) => message.ruleId === "worknaru/no-inline-style",
    ),
  );
  const [boundary] = await eslint.lintText(
...
test("style checks reject raw visual values and accept shared tokens", async () => {
  ...
  const bad = await stylelint.lint({
    code: ".example { color: #123456; padding: 13px; }",
    configFile,
  });
  assert.equal(bad.errored, true);
  const good = await stylelint.lint({
    code: ".example { color: var(--text); padding: var(--space-4); }",
    configFile,
  });
