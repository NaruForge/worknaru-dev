...
export async function startAgents(core: WorknaruCore) {
  ...
  el<HTMLFormElement>('send-form').onsubmit = async event => {
    ...
    try {
      ...
      if (request.state === 'failed' || request.state === 'uncertain') notice(request.error ?? '전송 결과를 확인해 주세요.');
      else { if (composer.value === text && selected?.id === id) composer.value = ''; pendingSend = null; modeOverride = false; mode.value = settings.sendMode; notice(request.mode === 'steer' ? '추가 지시를 전달했습니다.' : '대기열에 추가했습니다. 이 화면을 닫아도 실행됩니다.'); }
    } catch (e) { report(e); }
