...
Codex Agent 생성·조회·메시지 전송·응답 확인·후속 대화·보관을 제공한다. CLI와 같은 Agent·대기열·기본 전송 설정을 사용한다. 화면 위 **연결 확인**에서 Daemon 상태도 조회할 수 있다.
...
**전송 설정**은 CLI와 공유하는 유일한 전송 방식 설정이다. 메시지 입력창에는 현재 방식만 표시한다. 변경은 새로 접수한 메시지부터 적용하며 접수된 대기와 재시도 요청의 방식은 유지한다. 대기열은 성공한 작업 뒤에 순서대로 실행하고 탭을 닫아도 유지된다. 추가 지시는 Paseo 기본 동작을 따르며, 적용할 수 없으면 진행 중인 작업을 중단하고 새 요청을 시작할 수 있다.
...
작은 화면에서는 목록과 대화를 한 화면씩 표시한다. **Agent 목록으로** 돌아가도 탭 안의 초안은 유지된다. 화면 위 **화면: 시스템** 메뉴에서 밝게·어둡게·시스템 설정을 선택하며 브라우저에만 저장한다. 폼에는 label과 키보드 포커스를 제공하고 상태·오류는 텍스트로 안내한다. 대화·도구 출력은 HTML로 실행하지 않고 텍스트로 표시한다.

가장 작은 확인 순서는 다음과 같다.

1. **연결 확인 → 상태 확인**을 눌러 정상 응답 안내, 서버 ID와 `0.8.0` 버전 표시를 확인한다.
...
- [시작 코드](src/bootstrap.ts)가 Paseo Adapter를 만들고 Core에 주입한다. [Agent 화면](src/features/agents/AgentScreen.tsx)은 Core의 `agents` 메서드 객체를, [앱 화면](src/shell/App.tsx)은 `getDaemonStatus()`를 호출한다. 공통 표현은 [UI 패키지](../../packages/ui/README.md)를 조립한다.
