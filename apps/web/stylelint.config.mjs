  ...
  root.walkDecls((declaration) => {
    if (declaration.prop.startsWith("--")) {
      stylelint.utils.report({
        ...
        node: declaration,
        message:
          "Define tokens only in packages/ui/src/tokens.css; see docs/ui-design.md.",
      });
      return;
    }
    const rawColor = /#[\da-f]{3,8}\b|\b(?:rgb|hsl|oklch|color-mix)\(/i.test(
      declaration.value,
    );
    const rawSize = /(?:^|[\s,(])-?\d*\.?\d+(?:px|rem|em)\b/.test(
      declaration.value,
    );
    if (
      rawColor ||
