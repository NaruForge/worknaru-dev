...
대체 local Issue, 상태 라벨, 수동 진행표를 만들지 않는다. Project에 등록되지 않은 이슈의 상태는 미설정으로 보고하며 `Inbox`라고 추정하지 않는다. 설치 시의 설정 확인과 실제 Work Item의 lifecycle 검증을 구분한다. 실제 적용 근거는 해당 Issue와 Project에서 확인한다.
...
이 절의 설치 검증은 기록 체계 자체를 생성·변경할 때 적용한다. 이때 생성·변경한 기록 체계 파일을 다시 읽어 상대 링크, YAML·Form 구조, 종류·상태 매핑, 정보별 원본과 변경에 관련된 Receipt revision을 검사한다. 일반 제품 작업마다 전체 문서·Receipt·원격 설정을 다시 감사하지 않는다. Git 상태로 승인된 변경만 있는지 확인한다. 별도 validator나 검증 보고서 파일을 운영 산출물로 추가하지 않는다.

승인된 원격 변경 후에는 해당 변경 그룹의 라벨·Project·필드·뷰·workflow·저장소 연결 또는 Form 설정을 다시 읽는다. 설정 확인과 실제 lifecycle 사용 검증을 구분하며, 실제 흐름은 진짜 Work Item에서 확인한다. 설치 증명을 위한 가짜 이슈, placeholder, 빈 ADR, 임의 상태 이동은 만들지 않는다.
