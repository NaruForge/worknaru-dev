...
승인된 원격 변경 후에는 라벨·Project·필드·뷰·workflow·저장소 연결과 Form의 실제 설정을 변경 그룹마다 다시 읽는다. 설정 확인과 실제 lifecycle 사용 검증을 구분하며, 실제 흐름은 진짜 Work Item에서 확인한다. 설치 증명을 위한 가짜 이슈, placeholder, 빈 ADR, 임의 상태 이동은 만들지 않는다.
