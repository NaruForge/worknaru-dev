...
통합 설정은 **화면 / Agent 동작 / 연결**로 나눈다. 테마·배치는 로컬에 즉시 적용하고, 전송 방식은 **설정 → Agent 동작**에서 기존 Core의 revision 검증을 거쳐 명시적으로 저장한다. 목록과 입력창에는 전송 설정 바로가기를 두지 않으며 입력창에는 현재 방식만 표시한다. 미저장 상태로 이동하면 편집 내용과 표시를 유지하며 새로고침·탭 종료에는 브라우저 경고를 요청한다. 충돌 시 재조회 또는 취소로 복구한다. 연결은 진단만 제공한다.
