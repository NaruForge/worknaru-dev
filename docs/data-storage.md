...
Agent 요청의 컨텍스트 snapshot 도입으로 루트 저장 버전은 3이다. Agent 상태 문서 버전은 2다. 버전 1·2 루트는 DB를 열기 전에 `reset_required`로 거부하며 자동 변환·삭제하지 않는다. 기존 데이터를 보존하려면 새 외부 데이터 루트를 지정한다. 전체 초기화를 선택하면 아래 기존 보존/삭제 범위를 그대로 따른다.
