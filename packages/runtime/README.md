...
전송 방식은 공통 `settings`의 `queue`와 `steer`다. `send` 입력은 Agent·요청 ID·메시지와 선택적인 `target`을 받으며 요청별 mode를 받지 않는다. target 생략은 기존 호출 호환을 위한 standalone이며 새 호출은 명시적으로 지정한다. 응답의 `mode`는 접수된 실행 방식을 나타낸다. 설정 변경은 접수된 대기에 소급하지 않는다. steer는 Paseo 기본 동작을 따르며 미지원 시 기존 작업을 교체할 수 있다. 요청 상태는 `queued → sending → running → completed`이며 `failed`, `canceled`, `uncertain`도 별도로 표현한다. 성공한 턴 뒤에만 다음 대기를 실행한다. 재시작·통신 장애로 확정하지 못한 요청은 `uncertain`으로 남기며 자동 재전송하지 않는다. `discard`는 이를 사용자 확인으로 취소 처리하는 동작이며 완료나 Provider 실행 중단을 뜻하지 않는다.
...
[execution-context.ts](src/execution-context.ts)의 `ExecutionTarget`은 standalone, Workspace ID, Project ID 중 하나다. Project 요청에 Workspace를 동시에 지정하지 않는다. `ExecutionContext`는 type·workspaceId·projectId의 확정 snapshot이며 `AgentExecutionContext`와 Module 계약이 같은 형태를 사용한다. Standalone의 두 ID와 Workspace의 projectId는 null이다.

Core Resolver가 존재·소속을 검증하고 서버에서 `AgentRequest.context`를 확정한다. 요청별 snapshot은 저장·대기·재시도·Driver 전달에 사용한다. `parseExecutionTarget`/`parseExecutionContext`는 형식만 검사하며 존재/소속 검증이나 권한 부여를 대체하지 않는다. 실행 결과와 요청 조회는 항상 context를 포함한다. [결정](../../docs/adr/0017-agent-request-execution-context.md)을 따른다.
