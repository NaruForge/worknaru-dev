...
앱에서 호출하는 Worknaru API와 Agent 운영 정책이다. `createWorknaruCore({ runtime })`는 `getDaemonStatus()`와 타입 지정된 `agents(operation, input)`을 주입받은 [Runtime 계약](../runtime/README.md)에 전달한다.
