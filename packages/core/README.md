...
앱에서 호출하는 Worknaru API와 실행 정책이다. `createWorknaruCore({ runtime })`는 `getDaemonStatus()`, Agent 전용 `agents`, Workspace·Project 전용 `workspace`, Module 전용 `modules`를 주입받은 [Runtime 계약](../runtime/README.md)에 전달한다.
...
생성 호출은 호출마다 새로운 ID를 만든다. 전송 결과가 불명확한 생성 요청을 자동 재시도하지 않는다. 삭제·이동·수정, Agent/cwd 연결은 후속 범위다. 검증은 `pnpm test`가 타입·도메인·SQLite/재시작 테스트를 포함해 실행한다. 도메인 근거는 [Issue #51](https://github.com/NaruForge/worknaru-dev/issues/51), CLI·Web 연결은 [Issue #53](https://github.com/NaruForge/worknaru-dev/issues/53)에 있다.
